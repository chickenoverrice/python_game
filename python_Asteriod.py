...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated:
            index = (self.age % 64) // 1
            current_image_center = [self.image_center[0] +  index * self.image_size[0], self.image_center[1]]
            canvas.draw_image(self.image, current_image_center, 
...
def draw(canvas):
    ...
    score = score + 10 * group_group_collide(missile_group, rock_group)
    
    if lives == 0:
        started = False
        for i in range(len(rock_group)):
            rock_group.pop()
        for n in range(len(missile_group)):
            missile_group.pop()
        for m in range(len(explosion_group)):
            explosion_group.pop()
        timer.stop()
