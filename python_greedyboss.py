# http://www.codeskulptor.org/#user40_x04Ztoxko05GNA7.py
"""
...
def greedy_boss(days_in_simulation, bribe_cost_increment, plot_type = STANDARD):
    ...
    total_bribe = bribe
    bribe_day = 0
    current_day = int(math.ceil(float(bribe)/float(salary)))
    ...
    while current_day <= days_in_simulation:
        ...
        total_savings += (salary * (current_day - bribe_day))
        salary += 100
        bribe += bribe_cost_increment
        bribe_day = current_day
        #total_bribe += bribe
        # advance current_day to day of next bribe (DO NOT INCREMENT BY ONE DAY)
        current_day = current_day + int(math.ceil(float(bribe - (total_savings - total_bribe))/float(salary)))
        # update state of simulation to reflect bribe
        # update list with days vs total salary earned for most recent bribe
        total_bribe += bribe
        days_vs_earnings.append((bribe_day, total_savings))
